"""Run the authorized provider-free Repair20 regression gates once."""
...
from repair20_support import AUTOMATION_ROOT, PHASE2
...
def main() -> int:
    ...
    generic_code, generic = run_json(
        e2e.REPAIR_07_STAGE / "tests" / "run_fixture_generality_tests.py"
    )
    phase2_code, phase2 = run_json(PHASE2 / "run_systemic_regression_gate.py")
    output = {
        "classification": "REPAIR20 OFFLINE GATE RUN",
        "mandatory_preflight": {
        ...
        "genericity": {
            "returncode": generic_code,
            "passed": generic.get("passed"),
            "total": generic.get("total"),
        },
        "unified_phase2": {
            "returncode": phase2_code,
            "overall": phase2.get("overall"),
            "record_count": len(phase2.get("records", [])),
        },
        "provider_calls": 0,
        "executor_calls": 0,
        "role_calls": 0,
